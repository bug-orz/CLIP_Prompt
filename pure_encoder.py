...
class clip_image_encoder(nn.Module):
    ...
    def forward(self, input):
        x=self.clip_encoder(input)
        x=x.to(dtype=torch.float32)
        return self.fc(x)


clip_model.requires_grad=True
if os.path.exists('./save/fine_tuned_model'):
    model=torch.load('./save/fine_tuned_model')
else:
    model=clip_image_encoder(clip_model.encode_image)
...
for epoch in range(epochs):
    ...
    cur_acc=corr_num / (corr_num + err_num)
    if cur_acc>best_acc:
        torch.save(model, './save/fine_tuned_model')
        best_acc=cur_acc
    tqdm.write('Epoch {}, Top-1 Accuracy {}, Top-5 Accuracy {}'.format(epoch,top1,top5))
